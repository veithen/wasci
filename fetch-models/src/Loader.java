...
public class Loader {
        ...
        try {
            ...
            for (EObject object : (List<EObject>)resource.getContents()) {
                if (!object.eClass().getEPackage().getNsPrefix().equals("xmi")) {
                    result.add(convert(baseURI, object));
                }
            }
        } finally {
            in.close();
    ...
    private EObject convert(String baseURI, EObject in) throws Exception {
        if (in == null) {
            return null;
        }
        EClass inClass = in.eClass();
        EClass outClass = modelMapper.map(inClass);
        EObject out = EcoreUtil.create(outClass);
            ...
            if (fetch != null) {
                ...
                if (outRef.isMany()) {
                    ...
                    for (EObject inObject : (List<EObject>)in.eGet(inRef)) {
                        outCollection.add(convert(baseURI, inObject));
                    }
                } else {
                    out.eSet(outRef, convert(baseURI, (EObject)in.eGet(inRef)));
                }
            }
        }
        return out;
